...
class TestAnaFieldAnaRelation(unittest.TestCase):
    ...
    def test_creation(self):
        ...
        self.assertEqual(i2i1.to_dict(relation=True, mode='id'),
                         {'dist': 3, 'parentchild': False, 'relation': ['i2', 'i1'],
                          'typecoupl': 'derived'})
        self.assertEqual(i2i1.to_dict(relation=True, distances=True, mode='id'),
                         {'dist': 3, 'relation': ['i2', 'i1'], 'typecoupl': 'derived',
                          'parentchild': False, 'distance': 1, 'distomin': 0,
                          'distomax': 3, 'ratecpl': 0.25, 'rateder': 0.0})
    ...
    def test_tree(self):
        ...
        self.assertEqual(dts.tree('distomin', string=False),
                         {'-1': ['root-distomin (6)', {'0 ': ['i0 (0 - 6)', {
                             '1 ': ['i1 (0 - 3)']}, {'2 ': ['i2 (0 - 2)']}, {
                                 '3 ': ['i3 (0 - 2)']}, {'4 ': ['i4 (0 - 2)']}]}]})

    def test_partitions(self):
        '''tests AnaField, AnaRelation'''
        self.assertEqual(dts.partitions(
            mode='index', distributed=False), [[1, 3], [0]])
        self.assertEqual(dts.partitions(mode='id', distributed=False), [
                         ['i1', 'i3'], ['i0']])
...
class TestAnaDataset(unittest.TestCase):
    ...
    def test_sdataset(self):
        ...
        self.assertEqual(ilm.analysis.partitions(mode='index'), [
        ...
        self.assertEqual(ana.partitions(mode='index'), [
                         [0, 2, 5], [2, 5, 8], [0, 9], [2, 3], [8, 9], [7]])
        self.assertEqual(ana.field_partition(mode='index', distributed=False),
                         {'mixte': [3], 'primary': [0, 2, 5], 'secondary': [1],
                          'unique': [10], 'variable': [4, 6, 7, 8, 9]})
        ...
        self.assertEqual(ana.field_partition(mode='index', partition=ana.partitions()[1]),
                         {'mixte': [], 'primary': [2, 5, 8], 'secondary': [],
                          'unique': [10], 'variable': [0, 1, 3, 4, 6, 7, 9]})
        self.assertEqual(ana.field_partition(mode='field')
                         ['variable'], ana.variable)
        self.assertEqual(ana.field_partition(
            mode='field')['primary'], ana.primary)
        self.assertEqual(ana.field_partition(mode='field')
                         ['secondary'], ana.secondary)
        self.assertEqual(ana.field_partition(
            mode='field')['unique'], ana.unique)
    ...
    def test_partitions(self):
        ...
        self.assertEqual(ilm.analysis.field_partition(mode='index'),
                         {'mixte': [], 'primary': [0], 'secondary': [1, 2, 3, 4],
                          'unique': [], 'variable': []})
        ...
        self.assertEqual(AnaDataset(ilm.to_analysis(True)
                                    ).partitions(mode='index'), [])
